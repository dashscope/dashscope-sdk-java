...
public final class SpeechSynthesizerV2 implements AudioWebsocketCallback {
  ...
  private static final Integer DEFAULT_COMPLETE_TIMEOUT = 60 * 1000;
  ...
  private synchronized void sendTaskMessage(String action, JsonObject input) {
    ...
    log.debug("sendTaskMessage: {}", wsMessage.toString());
  ...
  public void onMessage(WebSocket webSocket, ByteBuffer bytes) {
    ...
    try {
      ...
      if (callback != null) {
        ...
        // Use atomic reference compare-and-swap for thread-safe accumulation
  ...
  private void accumulateAudioData(ByteBuffer frame) throws Exception {
    ...
    byte[] buffer = new byte[frame.remaining()];
    frame.get(buffer);
  ...
  public void streamingComplete() {
    streamingComplete(DEFAULT_COMPLETE_TIMEOUT);
  }
  ...
  public ByteBuffer call(String text, long timeoutMillis) throws RuntimeException {
    ...
    if (this.asyncCall) {
      ...
      return ByteBuffer.wrap(audioStream.toByteArray());
    }
  }
